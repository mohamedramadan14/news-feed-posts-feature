...
# create a fake file system  that fake an image file


@pytest.fixture()
def sample_image_file(fs) -> pathlib.Path:
    path = (pathlib.Path(__file__).parent / "assets" / "test_image.png").resolve()
    fs.create_file(path)
    return path
...
def mock_b2_upload_file(mocker):
    return mocker.patch(
        "socialmedia.routers.upload.b2_upload_file",
        return_value="https://fakeurl.com/test_image.png",
    )
...
async def test_upload_image(
    ...
    logged_in_token: str,
    sample_image_file: pathlib.Path,
):
    response = await call_upload_endpoint(
        async_client, logged_in_token, sample_image_file
    )
    assert response.status_code == 201
    assert "https://fakeurl.com/test_image.png" in response.json().get("file_url")
